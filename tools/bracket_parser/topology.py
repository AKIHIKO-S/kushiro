...
設計の前提(実データで確認):
  ...
  - 番号が無い表は罫線(リーフの水平線・物理行順)で順序を復元する設計だったが、
    api.py(呼び出し元)からは未接続のまま(2026-07-16確認)。現状は番号列が
    1つも見つからない表はその場で種目ごと諦め、呼び出し元(server.js)が
    JS副系統へフォールバックする。order_leaves/single_elim_treeは接続待ちの
    実装として残す(番号アンカー方式=entries.pyが正系という既存の設計判断は変えない)。

本モジュールが提供するもの:
  1. order_leaves       : リーフをドロー位置順(番号優先, 無ければ行)に整列。(未接続)
  2. single_elim_tree   : 整列済みリーフ→標準シングルエリミ木(1回戦=連番ペア)。(未接続)
