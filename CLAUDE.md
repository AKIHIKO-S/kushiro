...
- **所属履歴 (#298)**: `players.team` は「現所属キャッシュ」、正本は `affiliations`(player_id, team, kind, start_date, end_date='' で現所属・複数可)。`db.listAffiliations/currentAffiliations/affiliationAt(playerId,date)/addAffiliation/endAffiliation/backfillAffiliations`。createPlayer は team から初期1件を作り、`resolveBranchChange`(転校)は旧所属を締めて新所属を追加(上書きせず履歴を積む)。`MERGE_REPOINT` に登録済みでマージ/undo に追従。過去大会の「当時の所属」は `affiliationAt(id, 大会日付)` で復元。API: `GET/POST /api/players/:id/affiliations`・`PATCH /api/affiliations/:id/end`・`DELETE /api/affiliations/:id`・`POST /api/affiliations/backfill`(owner)。進学の自動繰り上げは未実装(履歴モデルは土台として用意)。回帰: `test/affiliation.test.js`。
