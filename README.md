...
**本番は Oracle Cloud (Always Free)** — systemd `ktta.service` + nginx。手順は [ORACLE_CLOUD_DEPLOY.md](./ORACLE_CLOUD_DEPLOY.md)、更新フローは [UPDATE_WORKFLOW.md](./UPDATE_WORKFLOW.md)、日常運用は [OPERATIONS.md](./OPERATIONS.md) を参照。

代替ホスティング: Fly.io / Railway 等の Docker 系 PaaS（同梱の `Dockerfile` を利用、SQLite 永続化に Volume が必要）、または国内 VPS(さくら/ConoHa)。GitHub Pages は静的のみのため単独では動かない。
