...
(function(global){
  ...
  function eventColor(event) {
    ...
    // 名前ハッシュ(既知パターンの同系統ゆらぎ用。未知種目のランダム色相生成には使わない)
    let hsh = 0; for (let i = 0; i < e.length; i++) hsh = (hsh * 131 + e.charCodeAt(i)) >>> 0;
    // 破壊的レビューで発覚: 旧実装はキーワード未一致の種目名(壮年/ジュニア/小学生等の自由入力)に
    // hue=hsh%360 で0-360度の完全ランダム色相を割り当てており、「壮年」と「ジュニア」が
    // 同じ濁った黄土色になる等、色分けの目的自体が機能不全を起こしていた。
    // 未知種目は彩度を大きく落としたニュートラルなスレートグレーに固定し、区別は
    // バッジのテキストラベルで行う(色数の上限を6系統+ニュートラル1に固定)。
    if (hue == null) {
      return { hue: 220, bg: "hsl(220, 12%, 46%)", fg: "#ffffff",
        border: "hsl(220, 12%, 34%)", soft: "hsl(220, 12%, 94%)" };
    }
    hue = (hue + (hsh % 18) - 9 + 360) % 360; // ±9°のゆらぎで同系統を区別
