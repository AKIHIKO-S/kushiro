...
test("区分別料金: buildAggregation は中高校生に fee_student を課金", () => {
  const t = { name: "区分", date: "2027-01-01",
    event_config: JSON.stringify([{ name: "男子シングルス", fee: 700, fee_student: 500 }]) };
  const entrants = [
    { event: "男子シングルス", team: "甲", category: "general", name: "A", gender: "male" },
    { event: "男子シングルス", team: "甲", category: "high", name: "B", gender: "male" },
  ];
  const { teams } = reports.buildAggregation(t, entrants, {});
  const members = teams.get("甲");
  assert.strictEqual(members.find((m) => m.category === "general").fee, 700, "一般=700");
  assert.strictEqual(members.find((m) => m.category === "high").fee, 500, "中高生=500(fee_student)");
  assert.strictEqual(members.reduce((s, m) => s + m.fee, 0), 1200, "団体合計=1200");
});

test("区分別料金: mailer.authoritativeFees は division=student に fee_student を適用", () => {
  const tournament = { event_config: JSON.stringify([{ name: "男子シングルス", fee: 700, fee_student: 500 }]) };
  const entries = [
    { event: "男子シングルス", type: "singles", division: "general", name: "A", fee: 0 },
    { event: "男子シングルス", type: "singles", division: "student", name: "B", fee: 0 },
  ];
  const { entries: out, total } = mailer.authoritativeFees(tournament, entries);
  assert.strictEqual(out[0].fee, 700);
  assert.strictEqual(out[1].fee, 500);
  assert.strictEqual(total, 1200, "合計は区分別で1200");
});
