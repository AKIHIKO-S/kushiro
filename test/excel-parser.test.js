...
test("looksLikeDoublesPlayers: 結合ペア氏名が過半なら doubles と判定", () => {
  // 結合ペア氏名(A/B・A・B)が過半 → true
  assert.ok(P.looksLikeDoublesPlayers([
    { name: "山田太郎/佐藤次郎" }, { name: "鈴木花子・田中一郎" }, { name: "単独 太郎" },
  ]), "結合ペアが2/3 → doubles");
  // 単独氏名ばかり(区切り無し)→ false
  assert.ok(!P.looksLikeDoublesPlayers([
    { name: "山田 太郎" }, { name: "佐藤 花子" }, { name: "鈴木 一郎" },
  ]), "単独氏名のみ → singles");
  // 1名に・が入るだけでは過半にならず singles のまま
  assert.ok(!P.looksLikeDoublesPlayers([
    { name: "A・B" }, { name: "佐藤 花子" }, { name: "鈴木 一郎" }, { name: "田中 二郎" },
  ]), "1/4 → singles のまま");
});

test("extractSheet: 採用範囲の欠番を埋める2件の端列は救済される", () => {
  // 主列に seed 1..6(密), 別列に 3,4 の欠番を埋める2件 → 救済して計8名
  const sheet = ws([
    [1, "選手 一", "(A)", "", "", 3, "選手 参", "(C)"],
    [2, "選手 二", "(B)", "", "", 4, "選手 四", "(D)"],
    [5, "選手 五", "(E)", "", "", "", "", ""],
  ...
  assert.ok(names.includes("選手 参") && names.includes("選手 四"), "欠番3,4を埋める列が救済される: " + names.join(","));
});

test("extractSheet: スコア由来の孤立2整数は救済しない(範囲外/欠番でない)", () => {
  // 主列 seed 1..4(密), 別列に範囲外の 11,12(スコアや別物) → 救済しない
  const sheet = ws([
    [1, "甲 一", "(A)", "", "", 11, "偽 参", "(X)"],
    [2, "乙 二", "(B)", "", "", 12, "偽 四", "(Y)"],
    [3, "丙 三", "(C)", "", "", "", "", ""],
  ...
  const players = P.extractSheet(sheet);
  const names = players.map(p => p.name);
  assert.ok(!names.includes("偽 参") && !names.includes("偽 四"), "範囲外の孤立整数は救済しない: " + names.join(","));
});
