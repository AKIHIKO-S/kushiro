...
test("F2: sanitizeFieldConfig が悪意/予約/型不正の custom を捨て、キーを英数字_に制限", () => {
  const t = db.createTournament({
    name: "無害化", date: "2027-11-10", event_config: singlesCfg, entries_open: 1,
    field_config: { version: 1, fields: { furigana: "required", bogus: "weird" }, custom: [
      { key: 'x" onfocus="alert(1)', label: "注入", type: "text", scope: "submission" },   // 属性インジェクション
      { key: "team", label: "予約衝突", type: "text", scope: "player" },                    // 予約キー
      { key: "bad", label: "型不正", type: "radio", scope: "submission" },                  // 想定外type
      { key: "tshirt", label: "OK", type: "select", options: ["S", "M"], required: true, scope: "player" },
    ] },
  });
  const rc = db.resolveFieldConfig(db.getTournament(t.id));
  const keys = rc.custom.map(c => c.key);
  assert.ok(!keys.includes('x" onfocus="alert(1)'), "属性インジェクションキーは除去");
  assert.ok(!keys.includes("team"), "予約キーは除去");
  assert.ok(keys.includes("tshirt") && keys.includes("bad"), "正当キーは残る");
  assert.strictEqual(rc.custom.find(c => c.key === "bad").type, "text", "想定外typeはtextに正規化");
  assert.strictEqual(rc.fields.bogus, undefined, "3値でないfields値は除去");
});

test("W1: ダブルスの選手ふりがな必須をサーバ側で検証(enforce)", () => {
  const t = db.createTournament({
    name: "複ふりがな", date: "2027-11-11", event_config: [{ name: "男子ダブルス", type: "doubles", fee: 1600 }],
    entries_open: 1, field_config: { version: 1, fields: { furigana: "required" } },
  });
  const base = { team_name: "甲", contact_name: "監督", contact_tel: "090", contact_email: "a@b.jp" };
  const ng = db.createTeamEntry(t.id, { ...base,
    entries: [{ event: "男子ダブルス", type: "doubles", name1: "山田", name2: "佐藤", team1: "甲", team2: "甲" }] }, "", { enforce: true });
  assert.ok(ng.error && /ふりがな/.test(ng.error), "ダブルスのふりがな欠落を弾く(片式だけでなく複でも)");
  const ok = db.createTeamEntry(t.id, { ...base,
    entries: [{ event: "男子ダブルス", type: "doubles", name1: "山田", name2: "佐藤", team1: "甲", team2: "甲",
      extra_json: { players: [{ furigana: "やまだ" }, { furigana: "さとう" }] } }] }, "", { enforce: true });
  assert.ok(ok.ok, "両者のふりがなが揃えば受付");
});

test("M1: 部分再送の併合で新しい引率者が既存原本へ反映される", () => {
  const t = db.createTournament({ name: "併合extra", date: "2027-11-12", event_config: singlesCfg, entries_open: 1 });
  const first = db.createTeamEntry(t.id, { team_name: "甲", contact_name: "監督", contact_email: "a@b.jp",
    supervisor: "旧引率", entries: [{ event: "中学男子シングルス", type: "singles", name: "選手A", team: "甲", fee: 500 }] });
  // 選手A(重複)+選手B(新規)を再送 → 選手B が first の原本へ併合
  const second = db.createTeamEntry(t.id, { team_name: "甲", contact_name: "監督", contact_email: "a@b.jp",
    supervisor: "新引率", entries: [
      { event: "中学男子シングルス", type: "singles", name: "選手A", team: "甲", fee: 500 },
      { event: "中学男子シングルス", type: "singles", name: "選手B", team: "甲", fee: 500 },
    ] });
  assert.ok(second.merged, "併合された");
  const view = db.getSubmissionByToken(first.applicant_token);
  assert.strictEqual(view.entries.length, 2, "旧トークンで全選手が見える");
  assert.strictEqual(view.extra.supervisor, "新引率", "併合で新しい引率者が原本に反映(取りこぼしなし)");
});
