...
test("getLastUpdated: 速報の更新で値が変わる(観戦ポーラーの変化検知が発火する)", () => {
  const t = setup(4);
  const m = firstReal(t.id);
  db.callMatch(m.id, 1);
  const before = db.getLastUpdated();
  db.setLiveScore(m.id, { s1: 1, s2: 0 });
  const after = db.getLastUpdated();
  assert.notStrictEqual(after, before, "速報だけの更新でも last-updated が変化する");
