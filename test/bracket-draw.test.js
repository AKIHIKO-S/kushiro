...
test("通常大会(v6・for_mac準拠): シードランク=標準位置番号(1=上端・2=下端)", () => {
  const size = 16;
  ...
  assert.strictEqual(slotOfSeed[1], 0, "第1シードは最上端(for_mac標準)");
  assert.strictEqual(slotOfSeed[2], size - 1, "第2シードは最下端(for_mac標準)");
  assert.strictEqual(new Set(Object.values(slotOfSeed)).size, 8, "重複なし");
});

test("釧路式(SS大会・kushiroOrder): シード番号=紙の上から物理順(第1=上端・第2=上の山…最終=最下端)", () => {
  const size = 16;
  const list = ents(16, (i) => (i <= 8 ? i : 0), (i) => "T" + i); // seed1..8
  const { leaves } = db.computeDrawLeaves(list, size, mulberry32(1), { separateBy: "none", kushiroOrder: true });
  const slotOfSeed = {}; leaves.forEach((e, i) => { if (e && e.seed) slotOfSeed[e.seed] = i; });
...
test("シード配置は draw_seed に依らず固定(両方式とも決定的=for_mac同様)", () => {
  const size = 16;
  const list = ents(16, (i) => (i <= 8 ? i : 0), (i) => "T" + i);
  const seen = new Set();
  for (let seed = 1; seed <= 12; seed++) {
    const { leaves } = db.computeDrawLeaves(list, size, mulberry32(seed), { separateBy: "none", kushiroOrder: true });
    const slotOfSeed = {}; leaves.forEach((e, i) => { if (e && e.seed) slotOfSeed[e.seed] = i; });
...
test("スーパーシード: 最終シード(釧路式=最下端)のSS区画は最下端側に確保される", () => {
  ...
  const size = 16;
  const { leaves } = db.computeDrawLeaves(list, size, mulberry32(5), { separateBy: "none", kushiroOrder: true });
...
// ── 統合Undo(undoLast): 編集があれば編集を、無ければ抽選を取り消す ──

test("統合Undo: 編集無しなら種目の抽選を取り消す", () => {
  const t = db.createTournament({ name: "統合undo1", date: "2028-02-01" });
  const EV = "男子シングルス";
  db.updateEntrySettings(t.id, { entries_open: 1, event_config: [{ name: EV, type: "singles", fee: 0 }] });
  for (let i = 1; i <= 8; i++) db.createEntrant({ tournament_id: t.id, event: EV, name: "選手" + i, team: "T" + i, status: "confirmed" });
  assert.ok(db.drawSingleBracket(t.id, EV, { draw_seed: 7, drawn_by: "x" }).success, "抽選");
  const u = db.undoLast(t.id, EV);
  assert.strictEqual(u.kind, "draw", "編集無し→抽選を取消");
  assert.ok(u.ok, JSON.stringify(u));
  assert.strictEqual(db.getMatchesByTournament(t.id).filter(m => m.event === EV).length, 0, "抽選前(表なし)に戻る");
});

test("統合Undo: 編集があれば編集を取り消す(抽選は残す)・繰り返しで抽選まで戻れる", () => {
  const t = db.createTournament({ name: "統合undo2", date: "2028-02-02" });
  const EV = "男子シングルス";
  db.updateEntrySettings(t.id, { entries_open: 1, event_config: [{ name: EV, type: "singles", fee: 0 }] });
  for (let i = 1; i <= 8; i++) db.createEntrant({ tournament_id: t.id, event: EV, name: "選手" + i, team: "T" + i, status: "confirmed" });
  db.drawSingleBracket(t.id, EV, { draw_seed: 7, drawn_by: "x" });
  const r1 = db.getMatchesByTournament(t.id).filter(m => m.event === EV && m.bracket_round === 1).sort((a, b) => a.bracket_pos - b.bracket_pos);
  db.swapBracketSlots(t.id, EV, { pos: r1[0].bracket_pos, slot: 1 }, { pos: r1[1].bracket_pos, slot: 1 });
  const u1 = db.undoLast(t.id, EV);
  assert.strictEqual(u1.kind, "op", "編集有り→編集(入替)を取消");
  assert.ok(db.getMatchesByTournament(t.id).filter(m => m.event === EV).length > 0, "抽選は残る");
  // もう一度で今度は抽選が取り消される
  const u2 = db.undoLast(t.id, EV);
  assert.strictEqual(u2.kind, "draw", "編集を戻したら次は抽選");
  assert.strictEqual(db.getMatchesByTournament(t.id).filter(m => m.event === EV).length, 0, "抽選前に戻る");
});

test("統合Undo: 何も無ければエラー", () => {
  const t = db.createTournament({ name: "統合undo3", date: "2028-02-03" });
  const u = db.undoLast(t.id, "男子シングルス");
  assert.ok(u.error, "取り消せる操作がない");
});
