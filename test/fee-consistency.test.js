...
test("会計突合: 却下/受付待ち(rejected/pending)は領収書・集計に計上しない", () => {
  // confirmed 3件(=EXPECTED 3300) に、却下1件・受付待ち1件を足しても合計は変わらないこと。
  // 既定 status は 'confirmed' なので status 未設定の既存 entrants はこれまで通り計上される。
  const withRejected = entrants.concat([
    { event: "一般男子ダブルス", team: "X高", name: "却下 太郎", partner_name: "却下 次", is_doubles: true,
      division: "general", category: "general", fee: 2000, status: "rejected" },
    { event: "中学生男子シングルス", team: "X高", name: "保留 三", is_doubles: false,
      division: "middle", category: "middle", fee: 500, status: "pending" },
  ]);

  const rl = reports.buildReceiptsList(tournament, withRejected);
  assert.strictEqual(rl.grand_total, EXPECTED, "領収書合計は confirmed のみ(却下/保留を除外)");

  const aggTotal = aggregationTotal(reports.buildAggregationXlsx(tournament, withRejected));
  assert.strictEqual(aggTotal, EXPECTED, "集計表合計も confirmed のみ");

  // 却下者は領収書項目に出てこない(X高の合計が膨らまない)
  const x = rl.items.find(i => i.team === "X高");
  assert.strictEqual(x.total, EXPECTED, "団体合計に却下/保留が混ざらない");
});
