...
test("swapDoublesOrder: 種目内の全ダブルスで選手1↔選手2を一括入替(furigana含む・表追従)", () => {
  const t = db.createTournament({ name: "並び入替" + (++_seq), date: "2027-09-12" });
  db.createEntrant({ tournament_id: t.id, event: EV, seed: 1, is_doubles: 1,
  ...
  assert.ok(r && r.ok && r.swapped === 2, "2ペア入替: " + JSON.stringify(r));

  const A = db.getEntrants(t.id, EV).find(e => e.surname === "小山内");
  assert.ok(A, "選手1が小山内になったペアがある(前と入替)");
  assert.strictEqual(A.given_name, "花", "選手1の名も入替");
  assert.strictEqual(A.furigana, "おさない", "選手1のふりがなも入替");
  assert.strictEqual(A.team, "北陽", "選手1の所属も入替");
  assert.strictEqual(A.partner_surname, "前", "選手2が前に");
  assert.strictEqual(A.partner_furigana, "まえ", "選手2のふりがなが まえ に");

  const names = r1Names(t, EV);
  assert.ok(names.indexOf("小山内") >= 0, "表の表示名も入替を反映: " + names);
});

test("swapDoublesOrder: 団体entrant(partner空のis_doubles)・相方未入力は対象外=氏名/所属を壊さない", () => {
  const t = db.createTournament({ name: "団体保護" + (++_seq), date: "2027-09-13" });
  const ev = "男子団体";
  const team = db.createEntrant({ tournament_id: t.id, event: ev, seed: 1, surname: "釧路", given_name: "クラブ", team: "釧路クラブ", status: "confirmed" });
  const before = db.getEntrants(t.id, ev).find(e => e.id === team.id);
  assert.ok(before.is_doubles, "団体種目は buildEntrantNames で is_doubles=1 になる");
  db.generateBracket(t.id, ev, { regenerate: true });
  const r = db.swapDoublesOrder(t.id, ev);
  assert.strictEqual(r.swapped, 0, "団体は入替対象外(swapped=0)");
  const after = db.getEntrants(t.id, ev).find(e => e.id === team.id);
  assert.ok((after.name || "").indexOf("釧路") >= 0 && after.surname === "釧路", "団体entrantの氏名が壊れない: name=" + after.name + " surname=" + after.surname);
  assert.strictEqual(after.team, "釧路クラブ", "所属も不変");

  // 相方未入力の半端ダブルスも対象外
  const ev2 = "男子ダブルス";
  const half = db.createEntrant({ tournament_id: t.id, event: ev2, seed: 1, is_doubles: 1, surname: "単独", given_name: "太", team: "X", status: "confirmed" });
  db.createEntrant({ tournament_id: t.id, event: ev2, seed: 2, is_doubles: 1, surname: "ペア", given_name: "一", team: "Y", partner_surname: "ペア", partner_given_name: "二", partner_team: "Y", status: "confirmed" });
  db.generateBracket(t.id, ev2, { regenerate: true });
  const r2 = db.swapDoublesOrder(t.id, ev2);
  assert.strictEqual(r2.swapped, 1, "相方ありの1ペアのみ入替(半端は除外)");
  const halfAfter = db.getEntrants(t.id, ev2).find(e => e.id === half.id);
  assert.strictEqual(halfAfter.surname, "単独", "相方未入力の選手1は空にならない: " + halfAfter.surname);
});
