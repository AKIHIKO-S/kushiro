...
test("ブロック方式: 512枠=4ブロック横並び+下段に準決勝・決勝(紙面方式)", () => {
  const t = db.createTournament({ name: "表検証ブロック4", date: "2027-05-05" });
  // createTeamEntry は1回100件上限のため大規模は createEntrant 直
  for (let i = 1; i <= 300; i++) db.createEntrant({ tournament_id: t.id, event: EV,
    name: "選手" + String(i).padStart(3, "0"), team: "ク" + (i % 19), furigana: "せ" + String(i).padStart(3, "0") });
  db.generateBracket(t.id, EV, {});
  const buf = reports.buildBracketXlsx(t, db.getMatchesByTournament(t.id), db.getEntrants(t.id), { event: EV });
  const { vals } = readSheet(buf, EV.slice(0, 30));
  const text = allText(vals);
  for (const b of ["Ａブロック", "Ｂブロック", "Ｃブロック", "Ｄブロック"]) assert.ok(text.includes(b), b + "見出し");
  assert.ok(text.includes("（準決勝）") && text.includes("（決勝）"), "下段ラベル(準決勝/決勝)");
  const names = new Set(Object.values(vals).filter(v => /^選手\d{3}$/.test(String(v))));
  assert.strictEqual(names.size, 300, "全300選手が余白に記載: " + names.size);
  assert.strictEqual(Object.values(vals).filter(v => v === "ｂｙｅ").length, 0, "ｂｙｅ表記なし");
  // _import はブロック分割に関係なく正準512リーフ(ラウンドトリップ互換)
  const imp = readSheet(buf, "_import");
  const rows = Object.keys(imp.vals).filter(a => /^A\d+$/.test(a) && +a.slice(1) >= 3).length;
  assert.strictEqual(rows, 512, "_import=512リーフ: " + rows);
});

test("ブロック方式: 256枠=2ブロック(下段は決勝のみ)・128枠以下は従来1ブロック", () => {
  const t = db.createTournament({ name: "表検証ブロック2", date: "2027-05-05" });
  for (let i = 1; i <= 200; i++) db.createEntrant({ tournament_id: t.id, event: EV,
    name: "選手" + String(i).padStart(3, "0"), team: "ク" + (i % 13), furigana: "せ" + String(i).padStart(3, "0") });
  db.generateBracket(t.id, EV, {});
  const buf = reports.buildBracketXlsx(t, db.getMatchesByTournament(t.id), db.getEntrants(t.id), { event: EV });
  const { vals } = readSheet(buf, EV.slice(0, 30));
  const text = allText(vals);
  assert.ok(text.includes("Ａブロック") && text.includes("Ｂブロック"), "2ブロック見出し");
  assert.ok(!text.includes("Ｃブロック"), "Ｃブロックは無い");
  assert.ok(text.includes("（決勝）") && !text.includes("（準決勝）"), "下段は決勝のみ");
  // 小規模(既存setup: 12人=16枠)はブロック見出しを出さない=従来レイアウト
  const t2 = setup(12, 4);
  const buf2 = reports.buildBracketXlsx(t2, db.getMatchesByTournament(t2.id), db.getEntrants(t2.id), { event: EV });
  const s2 = readSheet(buf2, EV.slice(0, 30));
  assert.ok(!allText(s2.vals).includes("ブロック"), "16枠はブロック見出しなし(従来表示)");
});
