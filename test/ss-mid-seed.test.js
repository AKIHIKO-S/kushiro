...
test("外シードSSのみ(seed1,2・R3)は従来どおり両端区画(移設なし・回帰)", () => {
  const list = ents(24, (i) => i <= 8 ? { seed: i, entry_round: i <= 2 ? 3 : 1 } : {});
  const { leaves, warnings } = db.computeDrawLeaves(list, 32, mulberry32(11), { separateBy: "none" });
  const e1 = list.find(e => e.seed === 1), e2 = list.find(e => e.seed === 2);
  assert.strictEqual(leaves.indexOf(e1), 0, "第1シードは最上端のまま");
  assert.strictEqual(leaves.indexOf(e2), 31, "第2シードは最下端のまま");
