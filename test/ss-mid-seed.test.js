...
test("中シードSS(seed3・R3): 衝突しても降格せず同じ山内のw=4全空き区画へ移設", () => {
  ...
  const { leaves, warnings } = db.computeDrawLeaves(list, 32, mulberry32(7), { separateBy: "none", kushiroOrder: true });
...
test("外シードSS(釧路式=第1と最終シード・R3)は両端区画(移設なし・回帰)", () => {
  ...
  const { leaves, warnings } = db.computeDrawLeaves(list, 32, mulberry32(11), { separateBy: "none", kushiroOrder: true });
...
test("混在: 2回戦から登場のシード(w=2)+非シードbye(大きい山の1戦目=2回戦)が同居できる", () => {
  ...
  const { leaves } = db.computeDrawLeaves(list, 32, mulberry32(3), { separateBy: "none", kushiroOrder: true });
  // 全員が一度だけ配置・bye数=12
...
test("同一draw_seedで移設込みの配置が完全再現される", () => {
  ...
  const a = db.computeDrawLeaves(mk(), 32, mulberry32(42), { separateBy: "team", kushiroOrder: true });
  const b = db.computeDrawLeaves(mk(), 32, mulberry32(42), { separateBy: "team", kushiroOrder: true });
