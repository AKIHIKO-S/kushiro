...
test("上限保証: 1ブロック128名(総512名+SS32=1024枠)で構造OK・Ａ〜Ｄ均等・Excel出力可", () => {
  const t = db.createTournament({ name: "上限512", date: "2027-07-01" });
  db.updateEntrySettings(t.id, { entries_open: 1, event_config: [{ name: EV, type: "singles", fee: 0, open: true }] });
  const ids = mkEntrants(t, 512);
  const sq = new Database(process.env.DB_PATH);
  for (let r = 1; r <= 32; r++) {
    const local = ((r - 1) % 8) + 1;
    const er = (local === 1 || local === 8) ? 5 : 3;
  ...
  const v = db.validateBracketStructure(t.id, EV);
  assert.strictEqual(v.ok, true, JSON.stringify(v.issues).slice(0, 400));
  assert.strictEqual(v.summary.players, 512);
  assert.strictEqual(v.summary.ss_ok, 32);
  assert.strictEqual(v.summary.finals_fixed, true);
  assert.deepStrictEqual(v.summary.blocks, [128, 128, 128, 128], "各ブロック128名ちょうど");
  const reports = require("../reports");
  const buf = reports.buildBracketXlsx(db.getTournament(t.id), db.getMatchesByTournament(t.id), db.getEntrants(t.id), { event: EV });
  assert.ok(buf && buf.length > 10000, "紙式Excelが出力できる: " + (buf && buf.length));
});

test("上限保証: SSなし512名(強制確定)=各ブロック128名ちょうど・BYEゼロ", () => {
  const t = db.createTournament({ name: "上限512素", date: "2027-07-01" });
  db.updateEntrySettings(t.id, { entries_open: 1, event_config: [{ name: EV, type: "singles", fee: 0, open: true }] });
  mkEntrants(t, 512);
  const dr = db.drawSingleBracket(t.id, EV, { drawn_by: "検証", force: true });
  assert.strictEqual(dr.success, true, JSON.stringify(dr).slice(0, 200));
  const v = db.validateBracketStructure(t.id, EV);
  assert.strictEqual(v.ok, true, JSON.stringify(v.issues).slice(0, 300));
  assert.strictEqual(v.summary.size, 512, "512枠にちょうど収まる");
  assert.strictEqual(v.summary.byes, 0);
  assert.deepStrictEqual(v.summary.blocks, [128, 128, 128, 128]);
});

test("小規模SS大会でも必ずＡ〜Ｄ4ブロック(12名=16枠)", () => {
  const t = db.createTournament({ name: "小規模4B", date: "2027-07-01" });
  db.updateEntrySettings(t.id, { entries_open: 1, event_config: [{ name: EV, type: "singles", fee: 0, open: true }] });
  mkEntrants(t, 12);
  const dr = db.drawSingleBracket(t.id, EV, { drawn_by: "検証", force: true });
  assert.strictEqual(dr.success, true, JSON.stringify(dr).slice(0, 160));
  const v = db.validateBracketStructure(t.id, EV);
  assert.strictEqual(v.ok, true, JSON.stringify(v.issues).slice(0, 300));
  assert.deepStrictEqual(v.summary.blocks, [3, 3, 3, 3], "小規模でも4ブロック均等");
  assert.strictEqual(v.summary.finals_fixed, true);
});
