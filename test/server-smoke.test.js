...
test("(d) 抽選ドロー: HTTP で draw→再現性→両山Excel出力→認可", async () => {
  ...
  // 抽選(種固定で再現性を確認)
  const d1 = await adminPost(`/api/tournaments/${t.id}/bracket/draw`, { event: EV, draw_seed: 777, separate_by: "team" });
  ...
  const d2 = await adminPost(`/api/tournaments/${t.id}/bracket/draw`, { event: EV, draw_seed: 777, separate_by: "team" });
