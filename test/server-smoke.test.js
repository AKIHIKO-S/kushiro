...
test("(s) オーナー 全選手削除: 実施者名と件数の打鍵確認が必須・実行で自動バックアップ＋監査記録", async () => {
  ...
  // GET /api/players は 2026-07-17 に requireAdmin 化(note等の生データ保護)。鍵つきで読む。
...
test("(z) 割当表API: 下書き→編集→確定→旧経路編集でdirty→再確定 の一巡(案B P3)", async () => {
  const EV = "男子シングルス";
  const t = await adminPost("/api/tournaments", { name: "シートAPI検証", date: "2027-01-01" });
  await adminPut(`/api/tournaments/${t.id}/entry-settings`, { entries_open: 1, event_config: [{ name: EV, type: "singles", fee: 0 }] });
  for (const nm of ["席 一郎", "席 二郎", "席 三郎", "席 四郎"])
  ...
  // 編集(入替)→古いhashは409
  const r1 = await adminPost(`/api/tournaments/${t.id}/bracket/sheet/ops`,
    { event: EV, base_hash: d.draft.sheet_hash, ops: [{ op: "swap", a: 0, b: 3 }] });
  assert.ok(r1.ok, JSON.stringify(r1).slice(0, 100));
  const stale = await fetch(BASE + `/api/tournaments/${t.id}/bracket/sheet/ops`, { method: "POST", headers: akhead,
    body: JSON.stringify({ event: EV, base_hash: d.draft.sheet_hash, ops: [{ op: "swap", a: 1, b: 2 }] }) });
  assert.strictEqual(stale.status, 409, "古いbase_hashは409");
  // 確定→第1版・matchesが生成される
  const c = await adminPost(`/api/tournaments/${t.id}/bracket/sheet/confirm`, { event: EV, by: "検証" });
  ...
  // 下書き→再確定で第2版・dirty解消
  await adminPost(`/api/tournaments/${t.id}/bracket/sheet/draft`, { event: EV });
  const c2 = await adminPost(`/api/tournaments/${t.id}/bracket/sheet/confirm`, { event: EV, by: "検証", force: true });
  assert.ok(c2.ok && c2.rev_no === 2, "再確定=第2版: " + JSON.stringify(c2).slice(0, 100));
  const st2 = await fetch(BASE + `/api/tournaments/${t.id}/bracket/sheet?event=` + encodeURIComponent(EV),
    { headers: akhead }).then(r => r.json());
