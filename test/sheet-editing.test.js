...
test("previewSheetBracket: 下書きから書込なしで表を導出(統合エディタの表示用)", () => {
  const { t } = setup(8, false);
  const d = db.ensureDraftSheet(t.id, EV);
  // まだ確定していないので現物matchesはゼロ
  assert.strictEqual(db.getMatchesByTournament(t.id).filter(m => m.event === EV).length, 0, "書込前ゼロ");
  const pv = db.previewSheetBracket(t.id, EV);
  assert.ok(pv && !pv.error, JSON.stringify(pv).slice(0, 120));
  assert.ok(Array.isArray(pv.matches) && pv.matches.length >= 7, "8人分の木がプレビューされる");
  assert.strictEqual(pv.sheet_status, "draft");
  assert.strictEqual(db.getMatchesByTournament(t.id).filter(m => m.event === EV).length, 0, "プレビューは書込ゼロ");
  // 下書き編集(枠1と枠8の入替)がプレビューに反映される
  const init = d.seats.map(s => s.entrant_id);
  assert.ok(db.applySheetOps(t.id, EV, d.sheet_hash, [{ op: "swap", a: 0, b: 7 }]).ok);
  const pv2 = db.previewSheetBracket(t.id, EV);
  const r1 = pv2.matches.filter(m => m.bracket_round === 1).sort((a, b) => (a.bracket_pos || 0) - (b.bracket_pos || 0));
  assert.strictEqual(r1[0].player1_entrant_id, init[7], "入替がプレビューに反映(枠1)");
  assert.strictEqual(r1[3].player2_entrant_id, init[0], "入替がプレビューに反映(枠8)");
});
