...
test("プレビューの種で確定すると同一配置(プレビュー=確定の一致)", () => {
  ...
  const r = db.drawSingleBracket(t.id, EV, { draw_seed: p.draw_seed, drawn_by: "ops-01" });
...
test("draw_log: 確定で1行記録・引き直しで supersede 連鎖・担当コード保存", () => {
  const t = setup(8, 0);
  db.drawSingleBracket(t.id, EV, { draw_seed: 1, drawn_by: "ops-01" });
  let log = db.getDrawLog(t.id, EV);
  assert.strictEqual(log.length, 1, "1回目で1行");
  assert.strictEqual(log[0].status, "committed");
  assert.strictEqual(log[0].drawn_by, "ops-01", "担当コード");
  assert.strictEqual(log[0].draw_seed, 1);
  assert.ok(log[0].entrants_hash && log[0].leaves_hash, "ハッシュ封印");
  db.drawSingleBracket(t.id, EV, { draw_seed: 2, drawn_by: "ops-02" });
...
test("undoDraw: 引き直しを取り消すと前の抽選に戻る", () => {
  const t = setup(8, 0);
  db.drawSingleBracket(t.id, EV, { draw_seed: 100, drawn_by: "ops-01" });
  const first = r1Of(t);
  db.drawSingleBracket(t.id, EV, { draw_seed: 200, drawn_by: "ops-02" });
  const second = r1Of(t);
...
test("undoDraw: 最初の抽選を取り消すとブラケットが消える", () => {
  const t = setup(8, 0);
  db.drawSingleBracket(t.id, EV, { draw_seed: 5, drawn_by: "ops-01" });
...
test("確定封印の差分: 抽選直後は原配置のまま・手修正で差分検知", () => {
  ...
  assert.strictEqual(d.drawn_by, "ops-01", "担当コード");
  // 2スロットを手修正(入替) → 差分2件
