...
test("決勝T/順位別Tからの再帰生成は拒否される", () => {
  const t = setup(["A", "B"]);
  const r1 = db.generateLeaguePlayoff(t.id, EV + " 決勝T", { mode: "top", advance_n: 1 });
  assert.ok(r1.error && /再生成できません/.test(r1.error), "決勝Tを元にした生成は拒否: " + JSON.stringify(r1));
  const r2 = db.generateLeaguePlayoff(t.id, EV + " 2位T", { mode: "byrank", advance_n: 2 });
  assert.ok(r2.error && /再生成できません/.test(r2.error), "順位別Tを元にした生成は拒否");
});
